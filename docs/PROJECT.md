...
| **테스트 모드**: 배치된 경로 따라 물품 1개 흐름 확인 | ✅ 완료 | 입고→컨베이어/기계1→출고 경로 시연. 컨베이어 입력/출력(방향전환) 반영 |
| 시뮬레이션 모드: 고정 시나리오 1~2종 | ⬜ 미구현 | 현재 플레이스홀더만 |
| 기본 지표: 처리량, 평균 대기 시간 (간단 수치) | ⬜ 미구현 | 시뮬레이션 연동 후 |

**Phase 1의 범위**  
- 시뮬레이션은 “배치에서 설정한 대로 물자 이동을 보는 것”이므로, **Phase 1에서 물류 시스템의 구조·작동 방식·흐름을 온전히** 갖춰 둠.  
- 테스트(물품 1개)와 시뮬레이션(시간·다량·지표)이 **같은 흐름 규칙**을 쓰도록, Phase 1에서 그 규칙(flow 모델)을 완성하고, Phase 2는 시간축·처리량·지표·재생 UI만 추가.

**Phase 1 완료**  
- 테스트 모드(물품 1개 흐름) 구현. 구조·타입·스펙 정리됨.  
- 다칸 배치·출고(창고 품목 선택)·공통 flow 엔진 등 Phase 1에서 마무리할 항목은 [STRUCTURE.md](STRUCTURE.md) 기준으로 진행.

**전체 구조**  
- [docs/STRUCTURE.md](STRUCTURE.md): Phase 1/2 역할, 방향전환, 다칸 장비, 입출력 포트, 출고·입고·창고.
